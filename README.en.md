...
sushida-cache bundles no game files — it patches `Web.data.unityweb` in the browser to turn the
...
Download the latest `sushida-cache-*.zip` from [Releases](https://github.com/kyo5uke/sushida-cache/releases), unzip it, then:
...
directly without the Neutral logo. Look for `[sushida-cache]` lines in the DevTools Console.
